...
func Usergen() User {
	...
	prefixidx := rng.Intn(len(nameprefix))
	middleidx := rng.Intn(len(namemiddle))
	suffixidx := rng.Intn(len(namesuffix))
	...
	password := ""
	for i := 0; i < 8; i++ {
		password += string(passwordletters[rng.Intn(len(passwordletters))])
	}
	...
	gakunenidx := rng.Intn(len(gakunen))
...
func Teamgen() Team {
	...
	invitation_code := ""
	for i := 0; i < 16; i++ {
		invitation_code += string("0123456789abcdef"[rng.Intn(16)])
	}
